...
static const std::string LIBEXEC_DIR { std::string { PXP_AGENT_ROOT_PATH } + "/libexec" };
...
TEST_CASE("Modules::Apply", "[modules]") {
    SECTION("can successfully instantiate") {
        REQUIRE_NOTHROW(Modules::Apply(PXP_AGENT_BIN_PATH, MASTER_URIS, CA, CRT, KEY, CRL, "", 10, 20, MODULE_CACHE_DIR, STORAGE, LIBEXEC_DIR));
    }
}

TEST_CASE("Modules::Apply::hasAction", "[modules]") {
    Modules::Apply mod { PXP_AGENT_BIN_PATH, MASTER_URIS, CA, CRT, KEY, CRL, "", 10, 20, MODULE_CACHE_DIR, STORAGE, LIBEXEC_DIR };
...
TEST_CASE("Modules::Apply::purge purges old cached files", "[modules]") {
    ...
    Modules::Apply mod { PXP_AGENT_BIN_PATH, MASTER_URIS, CA, CRT, KEY, CRL, "", 10, 20, PURGE_MODULE_CACHE_DIR, STORAGE, LIBEXEC_DIR };
...
TEST_CASE("Modules::buildCommandObject", "[modules]") {
    SECTION("fails when CRL is empty") {
        ...
        Modules::Apply mod { PXP_AGENT_BIN_PATH, MASTER_URIS, CA, CRT, KEY, "", "", 10, 20, MODULE_CACHE_DIR, STORAGE, LIBEXEC_DIR };
